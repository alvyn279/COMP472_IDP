...
class DepthFirstSearchStrategy(SearchStrategy):
    ...
    def __init__(self, max_depth):
        ...
        self.open_list = []  # type: List[List[Board]]
    ...
    def execute(self, board: Board):
        ...
        while len(self.open_list) != 0:
            ...
            while self.current_depth <= self.max_depth:
                current_level_of_boards = self.open_list.pop()
                new_boards = []

                while len(current_level_of_boards) != 0:

                    board_to_test = current_level_of_boards.pop()
                ...
                if self.current_depth != self.max_depth:
                    self.open_list.append(new_boards)
                self.current_depth += 1
